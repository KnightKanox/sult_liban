...
object DecisionEngine {
    fun decide(scene: SceneContext, skills: SkillResults, sourceMode: SourceMode): DecisionResult {
        val risks = listOfNotNull(
            ...
            skills.price?.let {
                when {
                    it.availability != Availability.AVAILABLE -> null
        ...
        val scores = risks.map { when (it) { RiskLevel.LOW -> 0.2; RiskLevel.MEDIUM -> 0.55; RiskLevel.HIGH -> 0.9 } }
        ...
        val recommendation = if (risk == RiskLevel.LOW) Recommendation.BUY else Recommendation.DELAY
        val points = buildList {
            ...
            skills.impulse?.takeIf { it.signals.isNotEmpty() }?.let { add("检测到促销信号：${it.signals.joinToString()}") }
            skills.price?.takeIf { it.availability == Availability.AVAILABLE }?.let {
                add(if ((it.premiumRatio ?: 0.0) > 0) "当前价格高于参考区间" else "当前价格处于参考区间")
            }
        }.take(4)
        ...
        val summary = when (risk) {
            ...
            RiskLevel.LOW -> "当前预算与价格风险较低，仍建议按实际需求决定。"
        }
